...
SRC_DIR ?= $(abspath src)
# freedom-metal path
METAL_LIB ?= $(abspath freedom-metal)
...
RISCV_LDFLAGS += -Wl,-Map,$(SRC_DIR)/$(PROGRAM).map
...
#############################################################
# Clang LLVM IR generated
#############################################################
CLANG  := $(CLANG_PATH)/bin/clang
LLC    := $(CLANG_PATH)/bin/llc
LLI    := $(CLANG_PATH)/bin/lli

SRC_CLANG_DIR = $(SRC_DIR)/clang-output
PROGRAM_LL    = $(SRC_CLANG_DIR)/$(PROGRAM).ll
PROGRAM_O     = $(SRC_CLANG_DIR)/$(PROGRAM).o
PROGRAM_S     = $(SRC_CLANG_DIR)/$(PROGRAM).S

.PHONY: clang
clang:
	mkdir -p $(SRC_CLANG_DIR)
	$(CLANG) $(SRC_DIR)/$(PROGRAM).c --target=riscv32 -S -emit-llvm -O0 -o $(PROGRAM_LL) -I$(METAL_LIB)

.PHONY: llc
llc:
	$(LLC) $(PROGRAM_LL) -o $(PROGRAM_S)
	$(CLANG) -O -c $(PROGRAM_LL) --target=riscv32 -o $(PROGRAM_O)
...
PROGRAM_SRCS = $(wildcard $(SRC_DIR)/*.c) $(wildcard $(SRC_DIR)/*.h) $(wildcard $(SRC_DIR)/*.S $(wildcard $(SRC_CLANG_DIR)/*.o))
...
$(PROGRAM_ELF): \
	...
	$(RISCV_GCC) $(PROGRAM_O) $(RISCV_CFLAGS) $(RISCV_LDFLAGS) $(RISCV_LDLIBS) -o $(SRC_DIR)/$(basename $(notdir $@))
